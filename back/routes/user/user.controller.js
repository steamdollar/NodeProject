...
exports.oauthkakao = async (req, res) => {
  ...
  try {
    ...
    const { nickname, profile_image_url: userimg } =
    ...
    if (sql_result2.length !== 0) {
      ...
      const sql =
        "INSERT INTO user(userid,nickname,userimg,userpw,username,address,gender,phone,mobile,email,userintro) values(?,?,?,'','','','','','','','')";
...
exports.login = async (req, res) => {
  ...
  const sql =
    "SELECT userid, userimg, username, nickname, address, gender, phone, mobile, email, level from user where userid = ? and userpw = ?";
...
exports.profile = async (req, res) => {
  ...
  if (usernickname == writernickname || writernickname == undefined) {
    const sql =
      "SELECT userid,username,userimg,nickname,address,gender,phone,mobile,email,userintro from user where nickname=?;";
...
exports.update = async (req, res) => {
  ...
  const userimg = req.file.filename;
  const sql =
    "UPDATE user SET userpw=?, userimg=?, nickname=?, address=?, gender=?, phone=?, mobile=? WHERE userid=?";
  const param = [
    userpw,
    userimg,
    nickname,
...
exports.userlike = async (req, res) => {
  ...
  if (userid == writeruserid || writeruserid == undefined) {
    const sql =
      "SELECT c.idx, c.category, c.userid, c.nickname, c.title, c.content, c.date, c.hit, count(c.idx) likes, c.hidden from cate1 c left join cate1_like l on c.idx = l.m_idx where l.userid=? group by c.idx order by date DESC;";
    const param = [userid];
